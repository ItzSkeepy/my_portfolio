...
export function initLiquidEther(container, options = {}) {
  const {
    mouseForce = 20,
    cursorSize = 100,
    isViscous = true,
  ...
  class MouseClass {
    ...
    onDocumentMouseMove(event) {
      // Pour ce projet, on force à être intérieur car la page entière est couverte
  ...
  // --- SHADERS ---
  const face_vert = `attribute vec3 position; uniform vec2 boundarySpace; varying vec2 uv; precision highp float; void main(){ vec2 scale = 1.0 - boundarySpace * 2.0; vec3 pos = position; pos.xy = pos.xy * scale; uv = vec2(0.5)+(pos.xy)*0.5; gl_Position = vec4(pos, 1.0); }`;
  const line_vert = `attribute vec3 position; uniform vec2 px; varying vec2 uv; precision highp float; void main(){ vec3 pos = position; uv = 0.5 + pos.xy * 0.5; vec2 n = sign(pos.xy); pos.xy = abs(pos.xy) - px * 1.0; pos.xy *= n; gl_Position = vec4(pos, 1.0); }`;
  const mouse_vert = `attribute vec3 position; attribute vec2 uv; uniform vec2 center; uniform vec2 scale; uniform vec2 px; varying vec2 vUv; precision highp float; void main(){ vec2 pos = position.xy * scale * 2.0 * px + center; vUv = uv; gl_Position = vec4(pos, 0.0, 1.0); }`;
  ...
  class ExternalForce extends ShaderPass {
    ...
    init(simProps) {
      ...
      this.mouse = new THREE.Mesh(new THREE.PlaneGeometry(1, 1), new THREE.RawShaderMaterial({
        ...
        uniforms: {
          px: { value: simProps.cellScale }, force: { value: new THREE.Vector2() },
          center: { value: new THREE.Vector2() }, scale: { value: new THREE.Vector2(simProps.cursor_size, simProps.cursor_size) }
        }
      }));
    ...
    update(props) {
      ...
      const uniform = this.mouse.material.uniforms;
      uniform.force.value.set(forceX, forceY);
      uniform.center.value.set(Mouse.coords.x, Mouse.coords.y);
      uniform.scale.value.set(props.cursor_size, props.cursor_size);
      super.update();
    }
  }
  ...
  class Viscous extends ShaderPass {
    ...
    update({ viscous, iterations, dt }) {
      ...
      for (let i = 0; i < iterations; i++) {
        [fbo_in, fbo_out] = i % 2 === 0 ? [this.props.output0, this.props.output1] : [this.props.output1, this.props.output0];
  ...
  class Poisson extends ShaderPass {
    ...
    update({ iterations }) {
      let p_in, p_out;
      for (let i = 0; i < iterations; i++) {
        [p_in, p_out] = i % 2 === 0 ? [this.props.output0, this.props.output1] : [this.props.output1, this.props.output0];
  ...
  class Pressure extends ShaderPass {
    ...
    update({ vel, pressure }) {
      this.uniforms.velocity.value = vel.texture; this.uniforms.pressure.value = pressure.texture; super.update();
    }
  }
  ...
  class Simulation {
    ...
    init() {
      ...
      const opts = { type, minFilter: THREE.LinearFilter, magFilter: THREE.LinearFilter, wrapS: THREE.ClampToEdgeWrapping, wrapT: THREE.ClampToEdgeWrapping };
  ...
  class WebGLManager {
    constructor(props) {
      ...
      this.simulation = new Simulation({
        iterations_poisson: iterationsPoisson, iterations_viscous: iterationsViscous, mouse_force: mouseForce, resolution, cursor_size: cursorSize,
        viscous, isBounce, dt, isViscous, BFECC
      });
    ...
    pause() { this.running = false; if (rafRef) { cancelAnimationFrame(rafRef); rafRef = null; } }
  }
  ...
  window.addEventListener('resize', () => { if (resizeRafRef) cancelAnimationFrame(resizeRafRef); resizeRafRef = requestAnimationFrame(() => webgl.resize()); });
}
